/*
		...
		Calling the script:
			...
			default: 		nul = [this] execVM "common\LV\reinforcementChopper.sqf";
			custom: 		nul = [spot,exact,side,type,captive,patrol,target,direction,distance,precise,
								cycle,groupSize,skills,smoke,group,custom init,ID,MP] execVM "common\LV\reinforcementChopper.sqf";
	...
	EXAMPLE: 	nul = [player,false,2,3,false,true,player,"random",1000,true,false,8,0.75,[false,true,false,true],nil,nil,33,false] execVM "common\LV\reinforcementChopper.sqf";
*/
	...
	if(typeName _targetMarker == "ARRAY")then{ //TARGET is array
		if((!((_targetMarker select 0) in allMapMarkers))&&(!((_targetMarker select 0) in allGroups)))then{
				...
				if((_targetMarker select 0) == "PATROL")then{ ////USE patrol-vD.sqf
					{
						...
						nul = [_x,_cPosition,_cRadius,_doorHandling] execVM "common\LV\patrol-vD.sqf";
					}forEach units _grp2;
				};
			};
		}else{ //TARGET is array of Markers or Groups
			{
				...
				if(_cycle)then{
					nul = [_x,true] execVM "common\LV\LV_functions\LV_fnc_follow.sqf";
				}else{
					nul = [_x,false] execVM "common\LV\LV_functions\LV_fnc_follow.sqf";
				};
				sleep 2;
			}forEach units _grp2;
		};
		...
		if(_targetMarker in allMapMarkers)then{ /////TARGET is single Marker
			{ _x doMove getMarkerPos _targetMarker; } forEach units _grp2;
				...
				{ ////TARGET is single Unit/Object
					_x setVariable ["target0",_targetMarker,false];
					_x setVariable ["mDis0", 1000, false];
					nul = [_x] execVM "common\LV\LV_functions\LV_fnc_follow.sqf";
				}forEach units _grp2;
		};
	};
}else{
