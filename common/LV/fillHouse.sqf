/*
			...
			Calling the script:
					...
					default: nul = [this] execVM "common\LV\fillHouse.sqf";

					custom:  nul = [target, side, patrol, patrol type, spawn rate, radius, skills, group,
									custom init, ID] execVM "common\LV\fillHouse.sqf";
...
EXAMPLE: 	nul = [this, 2, true, 2, 50, 1, 0.75, nil, nil, 9] execVM "common\LV\fillHouse.sqf";
...
if(isNil("LV_nearestBuilding"))then{LV_nearestBuilding = compile preprocessFile "common\LV\LV_functions\LV_fnc_nearestBuilding.sqf";};
...
while{_i2 < _rat}do{
	...
	if(_patrol)then{
        	switch (_pType) do {
            		case 1: {
                		_pFile = "common\LV\patrol-vF.sqf";
            		};
            		case 2: {
                		_pFile = "common\LV\patrol-vG.sqf";
            		};
        	};
