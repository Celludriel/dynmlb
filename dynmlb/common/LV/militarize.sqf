/*
		...
		default: 	nul = [this] execVM "common\LV\militarize.sqf";

		custom:		nul = [target, side, radius, spawn men, spawn vehicles, still, men ratio, vehicle ratio,
							skills, group, custom init, ID] execVM "common\LV\militarize.sqf";
...
EXAMPLE: nul = [this,2,50,[true,true],[true,false,true],false,[10,0],0.1,[0.2,0.2,0.2,0.85,0.9,0.75,0.1,0.6,1,1],nil,nil,13] execVM "common\LV\militarize.sqf";
*/
	...
	for "_i" from 1 to _menAmount do{
		...
		if(!_still)then{
			if(_unitType in _menArray)then{
				nul = [_unit,_cPos,_radius,_doorHandling] execVM "common\LV\patrol-vD.sqf";
			}else{
